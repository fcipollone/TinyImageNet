...
'''
def load_tiny_imagenet_for_augmentation(path, dtype=np.float32, subtract_mean=False):
    ...
    # Next load training data.
    X_train, y_train, file_paths, file_names, boxes_files = [], [], [], [], []
    for i, wnid in tqdm(enumerate(wnids), total=len(wnids)):
        # To figure out the filenames we need to open the boxes file
        boxes_file = os.path.join(path, 'train', wnid, '%s_boxes.txt' % wnid)
        ...
        for j, img_file in enumerate(filenames):
            img_file = os.path.join(path, 'train', wnid, 'images', img_file)
            file_paths.append(os.path.join('train', wnid))
            file_names.append(os.path.join(img_file))
            img = imread(img_file)
            if img.ndim == 2:   ## grayscale file
                img.shape = (64, 64, 1)
            X_train_block[j] = img
...
def save_augmented_tiny_imagenet(path, dataset):
    X_train = dataset["X_train"]
    ...
    for i, file_path in tqdm(enumerate(file_paths), total=len(file_paths)):
        ...
        directory_path = os.path.join(path, file_path, 'images')
        file_names_doc = os.path.join(path, file_path, augmented.txt)
